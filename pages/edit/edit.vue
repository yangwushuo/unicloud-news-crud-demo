		...
		<form>
			...
			<view class="news-control">
				<view class="news-control-but">
					<van-button color="#2529d8" icon="edit" :loading="loading" :loading-text="loadingText" round
						@click="sumbit">
						修改
					</van-button>
				</view>
		...
		methods: {
			...
			sumbit() {
				...
				if (!this.newsImg.length) {
					...
					//不修改图片
					if (this.newsImg[0].isExist) {
						this.upArticle(this.newsImg[0].url);
					} else {
						//修改图片
						uniCloud.uploadFile({
